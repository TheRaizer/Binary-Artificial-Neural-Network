...
TRAINING_SET_PATH = "iris_data/IrisTrainingBinary.csv"
TEST_SET_PATH = "iris_data/IrisTestBinary.csv"
DIMS_PATH = 'dims/iris_dims.pkl'
THETA_PATH = 'thetas/iris_theta_binary.pkl'


def train_iris_binary(dims, alpha=0.1, iterations=5000, is_mini_batch=False, batch_count=16, lambd=0, decay_rate=0):
    """ Trains the model on the iris binary data set.

    Preconditions:
    dims: non-empty list of integers
    alpha: float > 0
    iterations: int > 0
    is_mini_batch: bool
    batch_count: int > 0
    ...
    # load the data sets
    X_train, df = svld.load_csv_sets(TRAINING_SET_PATH, (1, 2, 3, 4), ",", 1)
...
def test_iris_binary(dims=None, theta=None):
    """ Runs a neural network model on the iris binary test set
    ...
    X_test, df = svld.load_csv_sets(TEST_SET_PATH, (1, 2, 3, 4), ",", 1)
